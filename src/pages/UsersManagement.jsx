...
export default function UsersManagement() {
  ...
  async function fetchUsersAndStores() {
    setLoading(true);
    ...
    // Extraer Tiendas Disponibles para el desplegable
    const { data: storesData, error: storesErr } = await supabase
      .from('stores')
      .select('*');

    if (!storesErr && storesData) setStores(storesData);
    setLoading(false);
  }
  ...
  const handleSaveUser = async (e) => {
    ...
    // En un entorno productivo esto idealmente pasa por Auth de Supabase, 
    // pero para MVP poblamos en el profile directamente para usar las métricas del Dashboard.
    const newUserId = crypto.randomUUID(); // Generador nativo nativo del navegador
    ...
    const { error } = await supabase.from('profiles').insert({
       ...
       role: formData.role,
       store_id: formData.store_id || null, // Permite nulo si no selecciona
    });
    ...
    if (!error) {
       ...
       setFormData({ full_name: '', email: '', password: '', role: 'asesor', store_id: ''});
       fetchUsersAndStores(); // Freshear
    } else {
  ...
  const handleDeleteUser = async (userId) => {
    const confirmDelete = window.confirm("¿Seguro que deseas dar de baja a este colaborador? Perderá todos sus FitCoins y progreso.");
  ...
  return (
    <>
                ...
                {users.map(user => (
                  <tr key={user.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '16px' }}>
                       <span className={`badge ${user.role === 'asesor' ? 'primary' : 'warning'}`}>{user.role}</span>
                    </td>
      ...
      {/* Modal / Panel Superpuesto Registro Personal */}
      {isModalOpen && (
        <div style={{
                ...
                <form onSubmit={handleSaveUser}>
                  ...
                  <div className="input-group">
                     ...
                     <input 
                       ...
                       placeholder="daniela.soto@empresa.com" 
                       required
                     />
                  </div>
